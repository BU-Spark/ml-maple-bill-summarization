...
As for generating the final dataset with all 7000+ bills, we are looking into some web scraping solution because the API endpoint requires us to input specific bill numbers. We will also ask the LLM expert if they had a list/how they would fetch the bills. But in the case of web scraping, it shouldn’t be too hard to flip through the different pages and get the bill numbers, then feed all of them through the API, and generate our final csv with all the bills.
